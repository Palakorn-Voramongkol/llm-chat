...
/** Define a role inside an existing application (Zitadel project). Posts to
 * /api/projects/{projectId}/roles (NOT the home-project /api/roles). May 403 if
 * the service account doesn't own the project — surfaced as an error toast. */
export function RoleCreateDialog({
  ...
  async function onSubmit(values: FormValues) {
    try {
      await api.post(`/api/projects/${projectId}/roles`, {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="brand" size="sm" data-testid="app-role-add">Add role</Button>
      </DialogTrigger>
      <DialogContent>
