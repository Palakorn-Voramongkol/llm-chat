...
function renderCell(colId: string, user: User) {
  const cols = buildColumns({
    onEdit: vi.fn(), onDelete: vi.fn(), onLifecycle: vi.fn(), onGrants: vi.fn(), onKeys: vi.fn(),
  });
...
describe("user columns", () => {
  it("has the expected columns", () => {
    const ids = buildColumns({ onEdit: vi.fn(), onDelete: vi.fn(), onLifecycle: vi.fn(), onGrants: vi.fn(), onKeys: vi.fn() })
  ...
  it("fires onLifecycle('deactivate') from the row action menu", async () => {
    ...
    const cols = buildColumns({ onEdit: vi.fn(), onDelete: vi.fn(), onLifecycle, onGrants: vi.fn(), onKeys: vi.fn() });
